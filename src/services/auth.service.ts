...
class AuthService {
  ...
  public async signUp(userData: CreateUserDto): Promise<User> {
    ...
    const duplicateUser: User = await userRepository.findOne({ where: { email: userData.email } });
    if (duplicateUser) throw new HttpException(409, `You're email ${userData.email} already exists`);
